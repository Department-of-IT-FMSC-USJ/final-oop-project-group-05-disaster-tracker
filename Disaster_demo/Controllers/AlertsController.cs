...
namespace Disaster_demo.Controllers
{
    ...
    public class AlertsController : ControllerBase // Inherit from ControllerBase
    {
        ...
        [HttpGet("toResolve/{gnDivision}")]
        public async Task<IActionResult> GetAlertsByDivision(string gnDivision)
        {
            var alerts = await _alertServices.GetAlertsByDivisionAsync(gnDivision);
            return Ok(alerts);
        }
