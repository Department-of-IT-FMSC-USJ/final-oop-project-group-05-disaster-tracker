...
namespace Disaster_demo.Controllers
{
    ...
    public class VolunteerController : ControllerBase
    {
        ...
        [HttpGet("by-division")]
        public async Task<IActionResult> GetVolunteersByDivision([FromQuery] string gnDivision)
        {
            if (string.IsNullOrEmpty(gnDivision))
                return BadRequest("GN Division is required.");

            var volunteers = await _volunteerServices.GetVolunteersByGnDivisionAsync(gnDivision);
