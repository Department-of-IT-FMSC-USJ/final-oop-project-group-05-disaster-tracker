...
namespace Disaster_demo.Services
{
    public class SymptomsServices : ISymptomsServices
    {
        ...
        public async Task<List<Symptoms>> GetPendingReportsByDivisionAsync(string gnDivision)
        {
            return await _dbContext.Symptoms
                .Where(r => r.action == "Pending" && r.gn_division == gnDivision)
                .ToListAsync();
        }
        ...
        public List<Symptoms> GetPendingSymptomsByGnDivision(string gnDivision)
        {
            return _dbContext.Symptoms
                .Where(s => s.action == "Pending" && s.gn_division == gnDivision)
                .OrderByDescending(s => s.date_time)
                .ToList();
        }
        ...
        public async Task<List<Symptoms>> GetGnApprovedSymptomsByDistrictAsync(string district)
        {
            return await _dbContext.Symptoms
