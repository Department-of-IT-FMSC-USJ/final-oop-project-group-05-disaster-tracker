...
namespace Disaster_demo.Services
{
    public class AidRequestServices : IAidRequestServices
    {
        ...
        //public List<AidRequests> getPendingAidRequests()
        //{
        //    return _dbContext.AidRequests
        //        .Where(s => s.gnApprove == GnApprovalStatus.Pending)
        //        .OrderByDescending(s => s.date_time)
        //        .ToList();
        //}
        //public async Task<List<AidRequests>> getPendingAidRequest()
        //{
        ...
        public async Task<List<AidRequests>> getPendingAidRequests(string gnDivision)
        {
            var approved = await _dbContext.AidRequests
                .Where(s => s.gnApprove == GnApprovalStatus.Pending && s.gn_division == gnDivision)
                .OrderByDescending(s => s.date_time)
                .ToListAsync();
        ...
        public bool UpdateStatus(StatusUpdateModel model)
        {
            ...
            else if (string.Equals(model.Actor, "DMC", StringComparison.OrdinalIgnoreCase))
            {
                if (Enum.TryParse<DmcApprovalStatus>(model.Status, true, out var parsedDmcStatus))
                {
                    ...
                    {
                        var gnOfficer = _dbContext.GN_Officers
                            .FirstOrDefault(g => g.gn_division.ToLower() == aidRequest.gn_division.ToLower());

                        if (gnOfficer != null)
                        {
                            aidRequest.assign_gn = gnOfficer.user_id;
                        }
                    }
        ...
        public async Task<List<AidRequests>> GetAidRequestsByDistrict(string district)
        {
            var requests = await _dbContext.AidRequests
                .Where(a => a.district.ToLower() == district.ToLower()
                            && a.gnApprove == GnApprovalStatus.Approved
