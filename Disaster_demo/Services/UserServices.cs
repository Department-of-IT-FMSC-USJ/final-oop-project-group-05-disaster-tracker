...
namespace Disaster_demo.Services
{
    public class UserServices : IUserServices
    {
        ...
        public async Task<LoginResponseDTO?> LoginAsync(LoginDTO dto)
        {
            ...
            {
                var gnOfficer = await _dbContext.GN_Officers
                    .FirstOrDefaultAsync(g => g.user_id == user.user_id);

                if (gnOfficer != null)
                {
                    response.GnDivision = gnOfficer.gn_division;
                    response.FullName = gnOfficer.name;
                    response.ContactNo = gnOfficer.contact_no;
                    response.District = gnOfficer.district;
                }
            }
